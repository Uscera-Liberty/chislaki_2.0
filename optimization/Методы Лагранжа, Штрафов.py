# Многомерный метод: метод Ньютона с одномерным поиском (Свенн + золотое сечение)
# Одномерная оптимизация: метод Свенна + метод золотого сечения
# Методы условной оптимизации: внешние штрафы и модифицированный метод множителей
...
def swann(f, x0, h=1.0):
    """
    Метод Свенна для поиска интервала, содержащего минимум унимодальной функции.
    f    : функция одной переменной
    x0   : начальная точка
    h    : начальный шаг
    возвращает (a, b) такой, что минимум находится внутри [a, b]
    """
    a, fa = x0, f(x0)
    b, fb = x0 + h, f(x0 + h)
    if fb > fa:
...
def line_search(f, x, d, h0=1.0, tol=1e-10):
    """
    Полноценный одномерный поиск вдоль направления d.
    f       : многомерная целевая функция
    x       : текущая точка
    d       : направление спуска (вектор)
    h0      : начальный шаг для метода Свенна
    tol     : точность золотого сечения
    возвращает оптимальную длину шага t*
    """
    # функция одной переменной φ(t) = f(x + t*d)
    phi = lambda t: f([x[i] + t * d[i] for i in range(len(x))])
    # 1) находим интервал, содержащий минимум (метод Свенна)
    a, b = swann(phi, 0.0, h0)
    # 2) уточняем минимум на интервале (золотое сечение)
    t_opt = golden_section(phi, a, b, tol)
    return t_opt
...
def newton_method(f, x0, tol=1e-9, max_iter=200):
    ...
    Используется вычисление градиента и гессиана (численно),
    направление Ньютона d = -H^(-1) * g,
    затем точный одномерный поиск (Свенн + золотое сечение) вдоль d.
    """
    x = x0[:]
    for _ in range(max_iter):
        ...
        if norm_g < tol:
            break
        H = hess(f, x)
        d = solve_linear(H, [-gi for gi in g])   # ньютоновское направление
        if d is None or sum(di * gi for di, gi in zip(d, g)) >= 0:
            d = [-gi for gi in g]               # антиградиент, если не спусковое
        # точный одномерный поиск оптимального шага
        t = line_search(f, x, d, h0=1.0, tol=1e-10)
        # обновление точки
